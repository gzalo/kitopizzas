...
class MDLViewer:
    def __init__(self, folder=None):
        ...
        if folder is None:
            folder = os.getcwd()
        self.file_nav = FileNavigator(folder, '.mdl', self.width, self.height)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.button == 4:  # Scroll up
                    if not self.file_nav.show_file_list:
                        self.zoom = max(1.0, self.zoom - 2.0)
                elif event.button == 5:  # Scroll down
                    if not self.file_nav.show_file_list:
                        self.zoom += 2.0
                elif self.file_nav.show_file_list and event.button == 1 and event.pos[0] < 320:
                    # Click on file list - handled by FileNavigator
                    pass
                elif event.button == 1:
                    self.dragging = True
                ...
                if self.dragging and not self.file_nav.show_file_list:
    ...
    def draw_overlay(self):
        ...
        # Build extra info
        extra_info = None
        if not self.load_error and self.mdl.frames:
            frame_idx = int(self.frame_index) % len(self.mdl.frames)
            frame_name = self.mdl.frames[frame_idx].get('name', '')
            extra_info = f"Frame: {frame_idx + 1}/{len(self.mdl.frames)}"
            if frame_name:
                extra_info += f" ({frame_name})"

        controls = "Left/Right: navigate | L: file list | Drag: rotate | Scroll: zoom"
    ...
    def run(self):
        ...
        print("  Home/End - First/last file")
